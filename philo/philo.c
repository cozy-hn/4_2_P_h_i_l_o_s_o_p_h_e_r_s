...
static int thread_init(t_philo **philo, t_arg *arg)
{
	...
	while (++i <= arg->philo_num)
	{
		if (pthread_create(&(*philo)[i].thread_id, NULL, &philo_act, &(*philo)[i]) != 0)
		{
...
int	main(int ac, char **av)
{
	...
	if (philo_init(&philo, &arg) == 1 || thread_init(&philo, &arg) == 1 || monitoring(&philo, &arg) == 1)
		return (ft_error());
